...
export default function MyFriendsMap() {
  ...
  const handleAddFriend = async (friend) => {
    ...
    try {
      ...
      toast.success(`You have added ${friend.username} as friend! Check map above to see them.`);
    } catch (error) {
  ...
  const getMarkerIcon = (friendImage) => new Leaflet.Icon({
    iconUrl: friendImage,
    iconSize: [38, 38],
  });
  ...
  useEffect(() => {
    ...
    const getOtherUsers = async () => {
      try {
        const { data: { data } } = await axios.get(`${serverURL}/friends/others`, {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
        });
                  ...
                  >
                    <Popup className="">
                      <div className="flex flex-col w-full">
                        ...
                        <div>
                          <span className="font-bold text-green-500">current location:</span>
                          {' '}
                  ...
                  <div className=" h-48 w-full  relative z-20 p-3 rounded-2xl ">
                    <div className="flex h-full flex-col justify-between relative z-40 w-full">
                      <div className="h-3/4 flex flex-col ">
                        <Link to={`${window.location.origin}/profile/${user?.username}`} className="flex items-center">
                          <div>
                            <img className="w-10 h-10 rounded-full" src={user?.profileBanner} alt="profile" />
                          </div>
                          <span className="ml-3  bg-gray-400 text-white py-1 px-2 rounded text-xs">{user?.username}</span>
                        </Link>
                        <div className="text-xs mt-2">
                          <span className="text-green-500">current location:</span>
                          <span className="ml-3">{user?.currentLocation?.name}</span>
                        </div>
                      </div>
